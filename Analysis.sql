...
FROM (SELECT 
	...
	DENSE_RANK() OVER(ORDER BY c.customer_id, COUNT(*) DESC) AS rank
FROM customers AS c
JOIN orders AS o
...
(
	SELECT o.restaurant_id,
		...
		COUNT(
				CASE WHEN d.delivery_id IS NULL THEN 1 END
		) as not_delivered
	FROM orders AS o
	LEFT JOIN deliveries AS d
	ON d.order_id = o.order_id
...
SELECT 
	...
	order_item,
	total_orders,
	RANK() OVER(PARTITION BY order_item ORDER BY seasons) as rank
FROM (
	SELECT
		CASE 
